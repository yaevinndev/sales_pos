...
class Items extends MY_Controller {
	...
	public function lotsview($param=0){

		if($param!=0){
			$getdata= $this->items->getalertlots();
			$data = $this->data;
			$data["viewdata"] =$getdata;
			if($this->permissions('lot_edit')) 
			$lot_edit=true;
		if($this->permissions('lot_segment'))
			$lot_segment=true;
			$data["lot_edit"]=$lot_edit;
			$data["lot_segment"]=$lot_segment;
		}else{
			$getdata = $this->items->getlots();
			if($this->permissions('lot_edit')) 
				$lot_edit=true;
			if($this->permissions('lot_segment'))
				$lot_segment=true;	
	...
	public function checklots(){
		 $do = $this->items->updatelots();
		 echo $do;
	}
	...
	public function editlot(){
		...
		$result=$this->items_model->editlot_model($num_lote,$cant);
		echo  $result;

	}

	 public function segmentlot(){
		$this->load->model('items_model');
		$num_lote = $this->input->post('num_lote');
		$new_lote = $this->input->post('new_lote');
		$new_cant = $this->input->post('new_cant');
		$new_fecha = $this->input->post('new_fecha');
		$new_producto = $this->input->post('new_product');
		$result= $this->items_model->segmentlot_model($num_lote,$new_lote,$new_cant,$new_fecha,$new_producto);
	 	echo $result;
	}
}
