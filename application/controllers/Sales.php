...
class Sales extends MY_Controller {
	...
	public function settings()
	{
		$this->permission_check('sales_add');
		$data=$this->data;
		$check = $this->sales->check_bill_settings();
		 if ($check=="none"){
			 $data['prefijo']="";
			 $data['inicio']="";
			 $data['final']="";
			 ...
			 $data['prefijo']=$check[0]["prefijo"];
			 $data['inicio']=$check[0]["inicio"];
			 $data['final']=$check[0]["final"];
			 $data['resolucion']=$check[0]["resolucion"];
			 $data['disabled']="disabled";
		 }
		$data['page_title']=$this->lang->line('master');
		$this->load->view('setbill',$data);
	}
	public function savebillparams(){
		$result = $this->sales->save_params();
			echo $result;
	}
