...

{
...

	{
		...
		$query14=$this->db->query("SELECT count(l.id) as total_items  from lotesxvencimiento as l 
		join db_items on id_producto=db_items.id 
		WHERE fecha_vence < '$vence_en' and estado=0");

		$total_items= $query14->result_array();
		...
		$data['total_items_v']= $total_items[0]['total_items']==null ? 0: $total_items[0]['total_items'] ;
