...
class Purchase_returns_model extends CI_Model {
	...
	public function verify_save_and_update(){
		...
		for($i=1;$i<=$rowcount;$i++){
			...
			if(isset($_REQUEST['tr_item_id_'.$i]) && !empty($_REQUEST['tr_item_id_'.$i])){
				...
				$num_lote           =$this->xss_html_filter(trim($_REQUEST['num_lot_'.$i.'_6']));
				$fecha_ven           =$this->xss_html_filter(trim($_REQUEST['td_data_'.$i.'_7']));
 	...
 	public function fixlot_on_return($data){
		 try {
			$item= $data["item_id"];
			$lote=$data["num_lote"];
			$cantidad= $data["qty"];
	
			//ubicamos el lote afectado 
	
			$q1 = $this->db->query("SELECT * from lotesxvencimiento where id_producto=$item and num_lote='$lote'");
			
			$cantidad_actual = $q1->row()->cantidad;
			$cantidad_final =$cantidad_actual - $cantidad;
			if($cantidad_final==0){
				//eliminamos el lote 
				$q2 = $this->db->query("DELETE from lotesxvencimiento where id_producto=$item and num_lote='$lote'");

			}else{
				//modificamos el lote
				$q2 = $this->db->query("UPDATE lotesxvencimiento set cantidad =$cantidad_final where id_producto=$item and num_lote='$lote'");
			}
	...
	public function purchase_list($purchase_code){
		$q1=$this->db->select('*')->from('db_purchaseitems')->join('db_purchase','db_purchase.id=purchase_id','right')->where("purchase_code='$purchase_code'")->get();
		$result=null;	
		$rowcount =1;
		foreach ($q1->result() as $res1) {
			...
			$info['num_lote']= $res1->num_lote;
			$info['fecha_ven']= $res1->fecha_ven;
		...
		?>
            <tr id="row_<?=$rowcount;?>" data-row='<?=$rowcount;?>'>
               ...
               <td id="td_<?=$rowcount;?>_9"><input type="text" name="td_data_<?=$rowcount;?>_9" id="td_data_<?=$rowcount;?>_9" class="form-control text-right no-padding only_currency text-center" style="border-color: #f39c12;" readonly value="<?=$item_amount;?>"></td>
				...
				<!-- Num_Lote -->
				<td id="d_<?=$rowcount;?>_6">
				<input type="text" name="num_lot_<?=$rowcount;?>_6" id="num_lote_<?=$rowcount;?>_6" class="form-control text-right no padding only" value="<?=$num_lote;?>" readonly>
				</td>

				<td id="d_<?=$rowcount;?>_7">
				<input type="text" name="td_data_<?=$rowcount;?>_7" id="td_data_<?=$rowcount;?>_7" class="form-control text-right no padding only" value="<?=$fecha_ven;?>" readonly>
				</td>
	...
	public function get_producto_compra($ref_fact){
		$query=$this->db->query("SELECT item_id,purchase_qty,price_per_unit,tax_id,tax_type,discount_amt,num_lote,total_cost FROM db_purchaseitems 
		JOIN db_purchase ON db_purchase.id= purchase_id WHERE purchase_code='$ref_fact'");
		if ($query->num_rows()==0){
		return false;
		}else{
	...
	public function findsupplier($code){
		$query = $this->db->query("SELECT s.id from db_suppliers as s 
		join db_purchase as p on s.id=p.supplier_id 
		where p.purchase_code='$code'");
		$query=$query->row();
