	...
	public function verify_save_and_update(){
		...
		for($i=1;$i<=$rowcount;$i++){
			...
			if(isset($_REQUEST['tr_item_id_'.$i]) && !empty($_REQUEST['tr_item_id_'.$i])){
				...
				$num_lots			=$this->xss_html_filter(trim($_REQUEST['td_data_'.$i.'_numlote']));
				$fecha_ven			=$this->xss_html_filter(trim($_REQUEST['td_data_'.$i.'_fecha_ven']));
				$fecha_nueva= str_replace("/","-",$fecha_ven);

				$fecha_ven = date("Y-m-d",strtotime($fecha_nueva));
				...
				if(strlen($num_lots)>1){
					$numlote_entry  = array(
								'id_producto'		=> $item_id,
								'cantidad' 			=> $purchase_qty,
								'num_lote'			=> $num_lots,
								'fecha_vence'		=> $fecha_ven,
					);

					$q2 = $this->db->insert('lotesxvencimiento', $numlote_entry);
				}
				...
				if(!$q6){
					return "failed";
							...
							/**
			 * Se añade caso especial para los entrys de mercancia
			 * 
			 */
			if($purchase_status=="Entry"){
				$this->db->query("insert into db_stockentry(entry_date,item_id,qty,status) values('$CUR_DATE',$item_id,$purchase_qty,1)");				
			}
		...
		?>
            <tr id="row_<?=$rowcount;?>" data-row='<?=$rowcount;?>'>
              ...
              <!--  <td id="td_<?=$rowcount;?>_12"><input type="text" name="td_data_<?=$rowcount;?>_12" id="td_data_<?=$rowcount;?>_12" class="form-control no-padding text-center" value="<?=$item_profit_margin;?>" onkeyup="calculate_sales_price(<?=$rowcount;?>)" ></td> -->
               ...
               <!-- ADD button -->
							 <td id="td_<?=$rowcount;?>_numlote">
								 <input type="text" name="td_data_<?=$rowcount;?>_numlote" id="td_data_<?=$rowcount;?>_numlote" class="form-control text-center ven_lote">
							 </td>
								<!-- fecha_v -->
							 <td id="td_<?=$rowcount;?>_fecha_ven">
								 <div class="date"><input type="text" name="td_data_<?=$rowcount;?>_fecha_ven" id="td_data_<?=$rowcount;?>_fecha_ven" class="form-control datepicker"></div>
							 </td>
