...
class Sales_model extends CI_Model {
	...
	public function check_resolution(){
		$q1= $this->db->query("SELECT * FROM masterdocumentos");
		if($q1->num_rows()>0){
	...
	public function verify_save_and_update(){
	    ...
	    if($command=='save'){//Create sales code unique if first time entry
			 $resolucion = $this->check_resolution();
			 if($resolucion=="empty"){
				 ...
				 $last= $resolucion[0]["last"];
				 $prefijo= $resolucion[0]["prefijo"];
				 $sales_code=$prejijo."-".$last;
				 $this->db->query("UPDATE masterdocumentos set last=$last+1");
				 $sales_code=$prefijo."-".$last;
			 }
		...
		for($i=1;$i<=$rowcount;$i++){
			...
			if(isset($_REQUEST['tr_item_id_'.$i]) && !empty($_REQUEST['tr_item_id_'.$i])){
				...
				$num_lote           =$this->xss_html_filter(trim($_REQUEST['td_data_'.$i.'_6']));
				...
				//Send var a check_lots function		
				$this->check_lots($item_id,$sales_qty);
		...
		?>
            <tr id="row_<?=$rowcount;?>" data-row='<?=$rowcount;?>'>
               ...
               <td id="td_<?=$rowcount;?>_9"><input type="text" name="td_data_<?=$rowcount;?>_9" id="td_data_<?=$rowcount;?>_9" class="form-control text-right no-padding only_currency text-center" style="border-color: #f39c12;" readonly value="<?=$item_amount;?>"></td>
				<!-- num_lote -->
				<td id="td_<?=$rowcount;?>_6"><input type="text" name="td_data_<?=$rowcount;?>_6" id="td_data_<?=$rowcount;?>_6" class="form-control text-right no-padding  text-center" style="border-color: #f39c12;"  ></td>
	...
	public function check_lots($id_p,$qty){

		$query=$this->db->query("SELECT * FROM lotesxvencimiento where id_producto = $id_p and estado = 0 order by fecha_vence asc");
		if($query->num_rows()==0){
			return false;
		}
		else{
	...
	public function check_bill_settings(){
		   $q1 = $this->db->query("SELECT * FROM masterdocumentos");
			 if($q1->num_rows()==0){
				 return "none";
			 }else{
	...
	public function save_params(){
		   $prefijo = $_REQUEST["prev"];
			 $inicio = $_POST["start"];
			 $final = $_POST["end"];
			 $resolucion =$_POST["resolucion"];
			 $masterdocumento = array(
				 'documento' => "facturas",
				 'prefijo' => $prefijo,
				 'inicio' => $inicio,
				 'final' => $final,
			...
			$this->db->insert("masterdocumentos",$masterdocumento);
