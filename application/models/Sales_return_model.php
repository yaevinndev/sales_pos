...
class Sales_return_model extends CI_Model {
	...
	public function verify_save_and_update(){
		...
		for($i=1;$i<=$rowcount;$i++){
			...
			if(isset($_REQUEST['tr_item_id_'.$i]) && !empty($_REQUEST['tr_item_id_'.$i])){
				...
				$num_lote	    	=$this->xss_html_filter(trim($_REQUEST['td_data_'.$i.'_6']));//lote
				...
				//Send var a check_lots_return function		
				$this->check_lots_return($item_id,$return_qty,$num_lote);
	...
	public function sales_list($sales_code){
		$q1=$this->db->select('*')->from('db_salesitems')
									->join("db_sales","db_sales.id=sales_id","right")
	...
	public function return_row_with_data($rowcount,$info){
		...
		?>
            <tr id="row_<?=$rowcount;?>" data-row='<?=$rowcount;?>'>
			   ...
			   <td id="td_<?=$rowcount;?>_6"><input type="text" name="td_data_<?=$rowcount;?>_6" id="td_data_<?=$rowcount;?>_6" class="form-control text-right no-padding only_currency text-center" style="border-color: #f39c12;" readonly value="<?=$num_lote;?>"></td>
	...
	public function check_lots_return($id_p,$qty,$num_lote){

		$query=$this->db->query("SELECT * FROM lotesxvencimiento where id_producto = $id_p and num_lote='$num_lote'");
		if($query->num_rows()==0){
			return false;
		}
		else{
			...
			foreach ($result as $value) {
				...
				$nuevo_stock = $cantidad_lote+$qty;
				$num_l = $value["num_lote"];

			$q=$this->db->query("UPDATE lotesxvencimiento set cantidad=$nuevo_stock where num_lote='$num_l'and id_producto='$id_p'");
				return true;
	...
	public function getclient($code){
		$query = $this->db->query("SELECT s.id from db_customers as s 
		join db_sales as p on s.id=p.custumer_id 
