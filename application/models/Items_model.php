...
class Items_model extends CI_Model {
	...
	var $column_order = array( 'a.id','a.item_image','a.item_code','a.item_name','b.category_name','f.localization_name','c.unit_name','a.stock','a.alert_qty','a.sales_price','d.tax_name','d.tax','a.status','e.brand_name','a.tax_type'); //set column field database for datatable orderable
	var $column_search = array( 'a.id','a.item_image','a.item_code','a.item_name','b.category_name','f.localization_name','c.unit_name','a.stock','a.alert_qty','a.sales_price','d.tax_name','d.tax','a.status','e.brand_name','a.custom_barcode','a.tax_type'); //set column field database for datatable searchable
	...
	private function _get_datatables_query()
	{
		...
		$this->db->join('db_localizations as f','f.id=a.localization_id','left');
		...
		if(!empty($localization_id)){
			$this->db->where("a.localization_id",$localization_id);
		}
	...
	public function verify_and_save(){
		...
		$query1="insert into db_items(description,item_code,item_name,brand_id,category_id,sku,hsn,unit_id,alert_qty,
							...
							values('$description','$item_code','$item_name','$brand_id','$category_id','$sku','$hsn','$unit_id','$alert_qty',
									'$price','$tax_id','$purchase_price','$tax_type',$profit_margin,
									'$sales_price','$custom_barcode',
	...
	public function getlots(){
		$sql = "select num_lote, fecha_vence,cantidad,estado,item_name,id_producto from lotesxvencimiento join db_items on id_producto=db_items.id ";
		$result=$this->db->query($sql);
		return $result->result_array();
	}

	public function editlot_model($num_lote,$cant){

		$query=$this->db->query("SELECT * FROM lotesxvencimiento where num_lote ='$num_lote'");
		if ($query->num_rows()==0){
			...
			foreach ($result as $value) {
				$cantidad_lote = $value["cantidad"];
				$nuevo_stock = $cant;

			$query2=$this->db->query("UPDATE lotesxvencimiento set cantidad = '$nuevo_stock' where num_lote = '$num_lote'");
...
public function segmentlot_model($num_lote,$new_lote,$new_cant,$new_fecha,$new_producto){

	$query=$this->db->query("SELECT id,id_producto,cantidad,estado FROM lotesxvencimiento WHERE num_lote='$num_lote' and id_producto=$new_producto");
	if($query->num_rows()==0){
		return false;
	}else {
		$result=$query->result_array();
		foreach ($result as  $value) {
			...
			$cantidad_lote=$value["cantidad"];
			$estado=$value["estado"];
			$nuevo_stock=$cantidad_lote-$new_cant;

	if($nuevo_stock<0){
		return "badcant";
	}	else{
		$query2=$this->db->query("UPDATE lotesxvencimiento SET cantidad=$nuevo_stock WHERE num_lote='$num_lote' and id_producto=$new_producto");
			if (isset($query2)){
				$newlote_entry = array(

					'num_lote' => $new_lote,
					'id_producto' => $id_producto,
					'fecha_vence' => $new_fecha,
					'cantidad' => $new_cant,
				);
				$query3=$this->db->insert('lotesxvencimiento', $newlote_entry);
					return "success";
				}
			}
...
public function getalertlots(){
		...
		$vence_en= date('Y-m-d',strtotime($fecha_actual."+ 45 days"));
		$query14=$this->db->query("SELECT  num_lote, fecha_vence,cantidad,estado,item_name,id_producto from lotesxvencimiento 
		join db_items on id_producto=db_items.id 
		WHERE fecha_vence < '$vence_en' and estado=0");
		
		$data = $query14->result_array();
