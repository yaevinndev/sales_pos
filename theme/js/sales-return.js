...
function delete_sales_payment(payment_id){
 ...
 if(confirm("Do You Wants to Delete Record ?")){
  ...
  $("#return_assoc").keypress(function(event){
    if(event.which=="13"){
      event.preventDefault();
      var num_factura =$("#return_assoc").val();
       $.ajax({
          type:"POST",
          url: mypath,
          data:{
          ...
          success: function(result){
            $('#sales_table tbody').append(result);
            rowcount = $('#sales_table tr').length;
       ...
       $.ajax({
        ...
        success: function(result){
          if(result!=''){
            $("#custumer_id").val(result);            
            $("#custumer_id").trigger('change');
              blockselect(result);
          }
