...
function delete_stock_entry(entry_id){
 ...
 if(confirm("Do You Wants to Delete Record ?")){
  ...
  $(".acction").on('click',function(){
	num_lote =  $(this).attr("lot");
	producto =  $(this).attr("product");
  });



  function callEditlot(num_lote){
	var cant = $("#cant_lote").val();
  ...
  function callSegmentlot(num_lote,producto){
	//var num_lote =  $(".acction").attr("lot");
	//var producto =  $(".acction").attr("product");
	var new_lote =  $("#new_num_lote").val();
    var n_cant   =  $("#new_cant").val();
	var n_fecha  =  $("#new_fecha").val();
		...
		success: function(result){
			if(result=="success"){
				...
				toastr["warning"]("El lote no puede quedar en 0.");
			}
