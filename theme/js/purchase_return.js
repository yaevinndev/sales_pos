...
function delete_return_payment(payment_id){
 ...
 if(confirm("Do You Wants to Delete Record ?")){
  ...
  $( "#ref_purchase").keypress(function( event ) {
    if ( event.which == 13 ) {
       event.preventDefault();
       var ref_fact = $(this).val();
       ...
       $.ajax({
          ...
          success: function(result){
            if(result!=''){
              $('#purchase_table tbody').append(result);
            rowcount = $('#purchase_table tr').length;
            $("#hidden_rowcount").val(rowcount-1);
            success.play();
            $("#ref_purchase").attr('disabled','disabled');
            enable_or_disable_item_discount();
            }else{
              toastr["warning"]("La Factura indicada no existe, verifique");
            }
